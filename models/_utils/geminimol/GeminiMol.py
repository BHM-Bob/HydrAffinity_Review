# from https://github.com/Wang-Lin-boop/GeminiMol/blob/main/geminimol/model/GeminiMol.py
...
class SelfAttentionPooling(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_scores = torch.tanh(nn.Linear(embedding_size, self.attention_size, bias=False)(x))
        attention_weights = torch.softmax(nn.Linear(self.attention_size, 1, bias=False)(attention_scores), dim=1)
...
class BinarySimilarity(nn.Module):  
    ...
    def sents2tensor(self, input_sents):
        input_tensor = batch([smiles_to_bigraph(smiles, node_featurizer=self.atom_featurizer, edge_featurizer=self.bond_featurizer) for smiles in input_sents])
        return input_tensor
    
    def mols2tensor(self, input_mols):
        from dgllife.utils import mol_to_bigraph
        input_tensor = batch([mol_to_bigraph(mol, node_featurizer=self.atom_featurizer, edge_featurizer=self.bond_featurizer) for mol in input_mols])
        return input_tensor
    ...
    def fit(self, 
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, dataset_size, self.batch_size//2):
                ...
                for label in self.label_list:
                    labels = torch.tensor(rows[label].to_list(), dtype=torch.float32)
    ...
    def prop_decode(self, 
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(df), self.batch_size):
                ...
                pred = self.Decoder[predictor_name](features)
    ...
    def similarity_predict(self, shape_database, ref_smiles, ref_as_frist=False, as_pandas=True, similarity_metrics=None, worker_num=1):
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(features_list), worker_num*self.batch_size):
                ...
                query_features = torch.from_numpy(np.array(features_batch))
...
if __name__ == '__main__':
    ...
    import torch
    model = torch.load('../EHIGN_dataset/_pretrained/GeminiMol/GeminiMol.pt', map_location='cpu')

    # model = BinarySimilarity(model_name = '../EHIGN_dataset/_pretrained/GeminiMol', **params)
