                ...
                if residue.get_id()[0] == " ":
                    ...
                    for atom_name in aa_coords.keys():
                        if atom_name in residue:
                            ...
                            aa_coords[atom_name].append([np.nan, np.nan, np.nan])
    aa_seq = "".join([seq1(aa) for aa in seq])
    
        
    # aa means amino acid
    coords = list(zip(aa_coords['N'], aa_coords['CA'], aa_coords['C'], aa_coords['O']))
    coords = torch.tensor(coords) # [n_atoms, 4, 3]
    # fill the missing coords with the mean value of the atoms in the same residue
    # 计算每个残基内有效原子的均值
    residue_means = torch.nanmean(coords, dim=1, keepdim=True)  # [n_residues, 1, 3]
    # 创建每个原子的掩码（NaN的位置）
    nan_mask = torch.isnan(coords)
    # 用残基均值填充NaN位置
    coords = torch.where(nan_mask, residue_means.expand_as(coords), coords)
...
if __name__ == "__main__":
    cid = '1gpk'
    pdb_file = f"/home/pcmd36/Desktop/USER_NAME/LFH/_My_Progs/EHIGN_dataset/PDBbind_2019/pdb/{cid}/chainA.pdb"
    print(os.path.exists(pdb_file))
    graph = generate_graph(pdb_file)
    print(graph)